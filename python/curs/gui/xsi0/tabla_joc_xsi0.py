...
def actiune_buton(button_var, label_var, txt):
    ...
    # Cod care se va muta in modulul server - analiza joc.
    ...
    if (caracter_castigator == "x") or (caracter_castigator =="0"):
        ...
        label_var.configure(text = "Victorie: " + str(caracter_castigator) + 
            ", " + str(tip_castig) + " " + str(unde_castig))
        fa_la_victorie()
        # resetez aceste valori. Am vazut ca uneori raman setate. 
        # nu stiu explicatia
...
def joc_nou():
    ...
    row = 0
    for i in range(0, 9):
        row = i // 3;   # 3 butoane pe linie;    
                        # i // 3 impartire fara rest; 5 / 3 = 1; 7 / 3 = 2
                        # pe prima linie vom adauga butoane de control; 
                        # butoanele jocului incep de la linia 1, nu 0
                        
        col = i % 3;    # 3 butoane pe coloana;  i %  3 restul impartirii;   
                        # 5 / 3 = 2; 7 % 3 = 1
        
        eval("b_"+str(row) + str(col)+".configure(text = '', \
            image = img_nimic, state = 'normal')")
...
def fa_la_victorie():
    '''
        Se dezactiveaza toate butoanele;
    '''
    row = 0
    for i in range(0, 9):
        row = i // 3;   
...
# Va fi generata matricea pentru x si 0, intr-o maniera dinamica - bucla for
# si comenzile exec & eval + codul generat sub forma text.
# Varianta statica ar fi presupus sa scriem individual codul pentru fiecare 
# buton. 
# Pentru 9 butoane ar fi posibil. daca crestem dimesiunea tabelei de joc,
# numarul de butoane va fi n x n si fa fi aporape imposibil sa scriem individual
# codul pentru fiecare buton
# 
# Python perminte generarea dinamica de cod, folosing functiile eval si exec  
#
# Pe langa butoane interfata va mai contine:
# - bara de stare - care afiseaza informatii despre mutarea curenta, cine
#   a castigat, daca jocul s-a terminat
# - bara de meniu - unde se va afisa un meniu cu diverse optiuni - acum doar
#   resetare joc 
##############################################################################

# Fereastra principala - se creaza la apelul functiei 'Tk()' din modulul tikinter
# importat ca 'tk'
main_win = tk.Tk()
main_win.title("X si 0")

# Celelalte ferestre se creaza ca si 'copii' ai ferestrei principale
#
# Variabila 'main_win' este data ca primul parametru la apelul functiilor
# tk.Frame, tk.Button, tk.Label etc
#
# bara meniu
...
# afisare buton meniu in bara de meniu si apoi bara meniu in aplicatie
...
# Este important ca fisierul cu imaginea sa se gaseasca in calea specificata.
# De schimbat locatia - ex: '../imgs/mele/48x48/x.png' daca folositi alte
# imagini.
...
row = 0
for i in range(0, 9):
    row = i // 3;   # 3 butoane pe linie;    i // 3 impartire fara rest; 
                    # 5 / 3 = 1; 7 / 3 = 2
                    # pe prima linie vom adauga butoane de control; 
                    # butoanele jocului incep de la linia 1, nu 0
                    
    col = i % 3;    # 3 butoane pe coloana;  
                    # i %  3 restul impartirii;   5 / 3 = 2; 7 % 3 = 1

    # Generare butoane. 
    # Se foloseste 'exec' declaratie / statement / bloc cod 
    # adica textul care contine comana / comenzile python de executat cand
    # acesta contine '=': ceva = ceva.
    #
    # exec    considera textul data ca parametru cod Pyton si incearca sa-l 
    # ----    execute.
    #         Intoarte 'None' adica ignora orice valoare ar putea fi intoarsa la 
    #         execuarea blocului de cod
    #         Se poate executa si o expresie (vezi la eval ce inseamna expresie)
    #         Nu se retine valoarea rezultata din evaluarea expresiei
    #         
    #         Ex. x = exec("2 + 2")
    #             print(x) -> None
    #             exec("x = 2 + 2") -> None dar se atribuie valoare variabilei x
    #             print(x) -> 4
    # 
    b_var = "b_"+str(row) + str(col)
    exec(b_var+" = tk.Button(x_0_frm, text = '  ', image = img_nimic, \
        command = lambda: actiune_buton(" + 
        b_var + ", l1, '" + str(row) + str(col)+"'))"
    )
    
    # Afisare butoane. Se foloseste 'eval' expresie
    # Sirul cu codul python NU contine '=' 
    # eval - considera textul dat ca parametru o expresie - adica ceva ce poate
    # ----   sa apara in partea stanga a unu '=': ceva = expresie
    #        evalueaza expresia si intoarte rezultatul acesteia
    #        ex: x = eval("2 + 2")
    #            print(x) -> 4
    eval("b_" + str(row) + str(col) + ".grid(row = " + 
        str(row) + ", column = " + str(col) + ")"
    )
    
    #print("v_"+str(i)+" = ", eval("v_"+str(i)))

# afisare frame matrice X si 0
x_0_frm.grid(row = 1, column = 0)

# bara stare
l1 = tk.Label(main_win)
# afisare bara de stare
